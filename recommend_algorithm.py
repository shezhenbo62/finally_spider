...
parser_put.add_argument('userId', type=int, required=True, help='Need UserId Data')
...
class CF(object):
        ...
        for i in self.recommendList:
            for j in self.datas:
                if j[3] == i[1]:
                    ...
                    self.rows.append({'info_id': info_id, 'title': title, 'getCost': getCost})
                    break
    ...
    def run(self, userId):
        ...
        try:
            self.n = len(self.userDict.get(userId))
        # self.n = 10
...
# 设置路由
api.add_resource(TodoList, '/userId')
