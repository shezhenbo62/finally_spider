...
def run():
    ...
    # 增加create_time列和categort_id
    # 3为服饰类别
    yoho['categort_id'] = 3
    yoho['create_time'] = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    # 删除_id列
    ...
    # 部分字段json序列化
    yoho1 = yoho[['title', 'link', 'rmb_final_price', 'rmb_original_price']].to_dict('records')
    ...
    yohobuy_data.to_excel('C:/Users/Administrator/Desktop/yohobuy_result.xls', index=False)
    ...
    #     # 使用 execute()  方法执行 SQL
    #     sql = '''
    #     insert into d88_material (brand_name,product_atlas,discount,url,discounts_money,current_price,original_price,
    #     describtion,categort_id,create_time)
